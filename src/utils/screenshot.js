...
module.exports.makeScreenshot = (process.env.AT_MOCK_SCR === 'yes') ? () => Buffer.from(EMPTY_IMAGE, 'base64') : () => Promise.resolve()
  ...
  .then(res => {
    ...
    if (res.code !== 200) {
      ...
      log.error(`ESCR501-${res.code}`, `Error in response from screenshot capture request: ${JSON.stringify(res.packet.body)}`);
      throw new UIError(res.code, `Error during screenshot capture request: ${res.packet.body}`, `ESCR501-${res.code}`);
