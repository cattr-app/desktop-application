 ...
 * @return {Promise<Promise<Model<any, any>[]>|Boolean>}                     Synced (actualized) ptasks from local storage, or bool true
 */
module.exports.syncTasks = async (fetch = true, highlight = false, onlyActive = true) => {
  ...
  const taskOptions = { user_id: currentUser.id };
  if (onlyActive)
    taskOptions.active = 1;
