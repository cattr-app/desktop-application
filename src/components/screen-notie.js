...
tracker.on('interval-pushed', data => {
  ...
  if (osIntegration.isApplicationClosingNow || !osIntegration.window || !userPreferences.get('showScreenshotNotification'))
    return;

  // Check is screenshot exists
  if (data.screenshot)
    return notie.screenshotNotification(data.screenshot, data.interval);
